...
def convert(pcb, brd):
    ...
    if outline.IsClosed():
        brd.write(
            "{x} {y}\n".format(
                x=coord(outline_points[0].x), y=coord(outline_points[0].y)
            )
        )
    brd.write("\n")
    ...
    for net_item in net_items:
        brd.write(
            "{code} {name}\n".format(code=net_item.GetNet(), name=net_item.GetNetname())
        )
    brd.write("\n")
